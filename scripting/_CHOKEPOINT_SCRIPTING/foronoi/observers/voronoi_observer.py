...
try:
    ...
    from foronoi.visualization.visualizer import Presets, Visualizer
...
class VoronoiObserver(Observer, ABC):
    ...
    def update(self, subject: Algorithm, message: Message, **kwargs):
        if plt is None or Visualizer is None or Presets is None:
            raise RuntimeError(
                "VoronoiObserver requires the optional matplotlib-based "
                "visualization extras, which are not bundled here."
            )
