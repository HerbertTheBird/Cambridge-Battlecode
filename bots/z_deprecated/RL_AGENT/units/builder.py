...
from units.RL_HELPER import forward, parse_log_line_py, flatten_observation_py
...
def run():
    ...
    print(f"INFER STATE: <span style='color: #{mode.r:02x}{mode.g:02x}{mode.b:02x}'>{mode.desc}</span>")
...
def _get_next_log_file():
    """Find the lowest game number that doesn't exist yet."""
    i = 0
    while True:
        path = os.path.join(LOGS_DIR, f"game_{i}.log")
...
def inference_state():
    """
    Log the full observation state for RL.
    """
    global log_file_path, mode
    if log_file_path is None:
        log_file_path = _get_next_log_file()
    turn = rc.get_current_round()
    my_id = rc.get_id()
    log_str = [f"T={turn}|ID={my_id}|M={mode.name}|TITANIUM={rc.get_global_resources()[0]}"]

    # --- Vision tiles ---
    for tile in rc.get_nearby_tiles():
        env = rc.get_tile_env(tile)
        log_str.append(f"TILE|{tile.x},{tile.y}|ENV={env.name}")

    # --- Nearby units ---
    for uid in rc.get_nearby_units():
        try:
            team = rc.get_team(uid)
            etype = rc.get_entity_type(uid)
            pos = rc.get_position(uid)
            if pos is None:
                continue
            log_str.append(f"UNIT|TEAM={team}|POS={pos.x},{pos.y}")
        except GameError:
            continue
    ...
    for bid in rc.get_nearby_buildings():
        try:
            ...
            if (etype == map_info._ET_ROAD or etype == map_info._ET_MARKER):
                continue
            pos = rc.get_position(bid)
            hp = rc.get_hp(bid)
            log_str.append(f"BUILDING|TEAM={team}|TYPE={etype.name}|POS={pos.x},{pos.y}|HP={hp}")
        except GameError:
            print("STATE LOG ERROR, LIKELY FATAL", file=sys.stderr)

    # Write to file
    obs_str = " | ".join(log_str) + "\n"
    with open(log_file_path, "a") as f:
        f.write(obs_str)
        
    obs = parse_log_line_py(obs_str)
    x = flatten_observation_py(obs)
    pred_idx = forward(x, WEIGHTS)  # WEIGHTS is the pure Python dict
    mode = int_to_mode[pred_idx]  # map index to Mode enum
