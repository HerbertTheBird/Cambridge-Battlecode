...
attack_finalized_turns = 0
def check_attack():
    global next_attack_tile, attack_finalized_turns
    my_pos = rc.get_position()
    ...
    if attack_tiles:
        ...
        if (next_attack_tile == my_pos):
            return
        path = nav.calculate_path(next_attack_tile)
        if path is not None and attack_finalized_turns > 0:
            mode = Mode.RUSH_CORE
            check_rush_core()
            attack_finalized_turns = 0
        elif path is not None:
            attack_finalized_turns += 1
    elif attack_finalized_turns > 0:
        mode = Mode.RUSH_CORE
        check_rush_core()
        attack_finalized_turns = 0
    else:
    ...
    for pos in rc.get_nearby_tiles():
        ...
        for cardinal in cardinal_dirs:
            ...
            if (my_pos == candidate):
                ...
                if adjacent_empty:
                    # Move to the empty tile closest to enemy core
                    adjacent_empty.sort(key=lambda p: p.distance_squared(map_info.predicted_enemy_core))
                    nav.move_to(adjacent_empty[0])
                    moved = True
                    ...
                    if rc.can_build_sentinel(candidate, direction):
                        ...
                        return  # only build one sentinel per turn
                    elif rc.can_build_gunner(candidate, cardinal):
                        rc.build_gunner(candidate, direction)
                        rc.draw_indicator_dot(candidate, mode.r, mode.g, mode.b)
                        return  # only build one gunner per turn
                    break
        ...
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                ...
                if points_at_tile and adjacent_empty:
                    nav.move_to(random.choice(adjacent_empty))
...
def run_pre():
            ...
            if (my_pos == candidate):
                ...
                if adjacent_empty:
                    ...
                    if rc.can_build_sentinel(candidate, direction):
                        ...
                        rc.draw_indicator_dot(candidate, mode.r, mode.g, mode.b)
                        return  # only build one gunner per turn
                    elif rc.can_build_barrier(candidate):
                        rc.build_barrier(candidate)
                        rc.draw_indicator_dot(candidate, mode.r, mode.g, mode.b)
                        return  
                    break
    ...
    # --- Cut supply: destroy our own buildings feeding enemy sentinels ---
    # Attack builders are already near enemy territory, so this is especially relevant
    for pos in rc.get_nearby_tiles():
        building_id = rc.get_tile_building_id(pos)
        if building_id is None:
            continue
        ...
        # Found enemy sentinel — check cardinal neighbors for our buildings
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if adj_type in (EntityType.CONVEYOR, EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE,
                ...
                # If we're close enough, destroy it now
                if my_pos.distance_squared(adj) <= 2:
                    if rc.can_destroy(adj):
                        log(f"Cutting supply: destroying our {adj_type} at {adj} feeding enemy sentinel at {pos}")
                        rc.destroy(adj)
                        return
                    elif rc.can_fire(adj):
                        rc.fire(adj)
                        return
