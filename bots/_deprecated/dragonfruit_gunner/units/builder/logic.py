...
def find_intercept_pos(ct: Controller, my_pos: Position, my_team: Team, vc: VisionCache, threat_pos: Position, map_obj, enemy_only: bool = False, global_titanium: int = 0, enemy_core_pos: Position | None = None, is_core_threat: bool = False) -> tuple[Position | None, int]:
    ...
    for (bid, etype, pos) in conveyors:
        if is_enemy_armoured_conveyor(etype, ct.get_team(bid), my_team):
            continue
        is_fed = False
...
def get_blocked_sentinel_directions(intercept_pos: Position, ct: Controller, map_obj) -> set:
    ...
    for feeder_pos, feeder_type in map_obj.get_feeders(intercept_pos):
        if feeder_type == EntityType.BRIDGE and map_obj.has_valid_input_chain(feeder_pos):
            return set()
...
def try_upgrade_conveyor(player, ct: Controller, my_pos: Position, vc: VisionCache) -> bool:
    ...
    for direction in DIRECTIONS:
        ...
        if ct.get_team(bid) != player.my_team or ct.get_entity_type(bid) != EntityType.CONVEYOR:
            continue
        ...
        if output_pos is None:
            continue
        output_entity = player.map.get_tile_entity(output_pos)
        if (
            output_entity is None
            or output_entity[2] != player.my_team
            or output_entity[1] in (EntityType.ROAD, EntityType.MARKER)
        ):
            continue
...
def find_sabotage_target(player, ct: Controller, my_pos: Position, vc: VisionCache, sabotage_worthy_ally_positions: set[Position] | None = None) -> tuple[Position, int] | None:
    ...
    If sabotage_worthy_ally_positions is provided, it will be populated with
    ally positions that have downstream_priority >= 2 (i.e. feed the enemy)."""
    ...
    for (bid, etype, pos) in consider:
        ...
        if is_ally and downstream_priority >= 2 and sabotage_worthy_ally_positions is not None:
            sabotage_worthy_ally_positions.add(pos)
...
def update_broken_chains(player, ct: Controller, vc: VisionCache) -> None:
    ...
    for (bid, etype, pos) in vc.ally_conveyors:
        ...
        player.broken_chains[output_pos] = _infer_broken_chain_resource(player, ct, bid, pos)
...
def find_heal_target(player, ct: Controller, my_pos: Position, vc: VisionCache, sabotage_worthy_ally_positions: set[Position] | None = None) -> Position | None:
    ...
    for (_eid, etype, epos) in vc.enemy_units:
        ...
        if ct.get_team(bid) != player.my_team:
            continue
        if sabotage_worthy_ally_positions and epos in sabotage_worthy_ally_positions:
            continue
        btype = ct.get_entity_type(bid)
...
def sync_a_star_destination(player) -> bool:
