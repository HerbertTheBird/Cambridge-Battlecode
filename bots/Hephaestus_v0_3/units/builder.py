...
def check_heal_core():
    pass
def run_heal_core():
    """Heal the core and fight enemy builders nearby."""
    if map_info.my_core is None:
        return
    my_pos = rc.get_position()
    core = map_info.my_core

    # Heal core if adjacent
    if my_pos.distance_squared(core) <= 2 and rc.can_heal(core):
        rc.heal(core)
        return

    # Attack enemy builders near our core
    for unit_id in rc.get_nearby_units(dist_sq=8):
        if rc.get_team(unit_id) == rc.get_team():
            continue
        if rc.get_entity_type(unit_id) != EntityType.BUILDER_BOT:
            continue
        enemy_pos = rc.get_position(unit_id)
        if enemy_pos.distance_squared(core) <= 8:
            # Enemy builder near our core! Attack it
            if my_pos.distance_squared(enemy_pos) <= 2 and rc.can_fire(enemy_pos):
                rc.fire(enemy_pos)
                return

    # Move to core if not adjacent
    if my_pos.distance_squared(core) > 2:
        nav.move_to(core)