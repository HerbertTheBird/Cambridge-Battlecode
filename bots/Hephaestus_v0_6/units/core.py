...
def run():
    ...
    # Normal spawning logic - lower threshold for early builders to boost exploration
    resource_threshold = 400 if num_spawned < 5 else 900
    if num_spawned < max_spawn or rc.get_global_resources()[0] > resource_threshold + rc.get_scale_percent() or rc.get_hp() < 500:
