...
MAX_SCORE = 8  # was 4 — boosted because mid-construction can score up to 8
...
def score():
    ...
    # outscore route/secure so we finish the ore before chasing other
    ...
    if last_flag == comm_flag and last_idx >= 0:
        if (1 << last_idx) & _cached_claims and _is_mid_construction(last_idx):
            return 8  # above route(7.75), secure(7.5)
    return 4
