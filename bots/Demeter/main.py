...
from bfs_nav import BFSNavigator
from map import Map
    ...
    def run(self, ct: Controller) -> None:
        try:
            ...
            if not self.initialized:
                ...
                self.bfs_nav.path_color = bot_path_color(self.my_id)
                
                self.map = Map(ct.get_map_width(), ct.get_map_height())
                self.nav.set_statics(self.map.width, self.map.height, self.my_id)
                self.bfs_nav.set_statics(self.map.width, self.map.height, self.my_id, ct.get_team())
