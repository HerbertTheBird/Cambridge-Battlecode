...
class VisionCache:
    __slots__ = (
        ...
        'harvesters', 'ally_builder_mask', 'enemy_builder_mask',
    )
    ...
    def __init__(self):
        ...
        self.harvesters = []
        self.ally_builder_mask = 0
        self.enemy_builder_mask = 0
        ...
        for eid in ct.get_nearby_entities():
            ...
            if team == my_team:
                if etype is _CORE:
                    ...
                    ab_a((eid, pos))
                    self.ally_builder_mask |= 1 << (pos.x + pos.y * width)
            ...
            # Add enemy entities
                ...
                if etype is _CORE or etype is _GUN or etype is _SEN or etype is _BRE or etype is _BB:
                    eu_a((eid, etype, pos))
                    if etype is _BB:
                        self.enemy_builder_mask |= 1 << (pos.x + pos.y * width)
                elif etype is _LAU:
                    el_a((eid, etype, pos))
                elif etype is _CON or etype is _AC or etype is _BRI or etype is _SPL:
                    ec_a((eid, etype, pos))
    ...
    def remove_entity(self, player, entity_id, entity_type, team, pos):
        ...
        bit = 1 << (pos.x + pos.y * player.map.width)
