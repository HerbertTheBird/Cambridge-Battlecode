...
class Comms:
    def __init__(self):
        ...
        self.launch_orders: list[tuple[int, int, int | None, int, int]] = []
    ...
    def decode_launch_order_target(self, msg: int) -> Position:
    ...
    def read_marker(self, msg: int, pos: Position | None = None, marker_id: int | None = None, current_round: int | None = None):
        if ((msg >> LAUNCHER_BIT) & 1) and ((msg >> LAUNCHER_ORDER_BIT) & 1):
            ...
            self.launch_orders.append((msg, builder_id, marker_idx, marker_id, current_round))
            return
        sym_bits = msg & 0x3
        if sym_bits != 0:
