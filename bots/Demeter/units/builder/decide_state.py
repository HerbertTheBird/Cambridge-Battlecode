...
def decide_state(player, ct: Controller, my_pos: Position, vc: VisionCache) -> State:
    ...
    sd_result = find_sabotage_target(player, ct, my_pos, vc) if player.global_titanium >= 20 else None
    ...
    heal_pos = find_heal_target(player, ct, my_pos, vc, sabotage_worthy_ally_mask)
    ...
    if player.core_pos is not None and ct.is_in_vision(player.core_pos):
        core_id = player.map.get_tile_entity_id(player.core_pos)
        if (
            core_id is not None
    ...
    player.nearest_unserviced = player.map.get_nearest_unserviced_harvester(my_pos, ct, player.core_pos)
    if player.nearest_unserviced is None:
        player.nearest_unharvested = player.map.get_nearest_ore_without_harvester(my_pos, ct, player.core_pos) if player.nearest_unserviced is None else None
