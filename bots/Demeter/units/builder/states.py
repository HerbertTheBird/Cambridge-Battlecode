...
def run_start_harvest_chain(player, ct: Controller, vc) -> None:
    ...
    if not is_ore_unblocked(player, ct, ore_pos):
        clear_state(player)
        player.map.add_unreachable_harvester(ore_pos)
        log(f"marked {ore_pos} as unreachable due to barriers")
        return

    ore_etype = player.map.get_tile_entity_type(ore_pos)
    ore_team = player.map.get_tile_entity_team(ore_pos)
    if ore_etype == EntityType.HARVESTER:
        ...
        if is_ore_unblocked(player, ct, ore_pos, allow_out_of_vision=False):
            best_build_pos = get_best_bridge_build_pos(
                ...
                player.map,
                vc,
                opposite_ore_mask=get_opposite_ore_mask(player.map, player.map.is_axionite_ore(ore_pos)),
            )
            ...
            player.harvest_ore_type = ResourceType.RAW_AXIONITE if player.map.is_axionite_ore(ore_pos) else ResourceType.TITANIUM
    ...
    is_titanium_ore = player.map.is_titanium_ore(ore_pos)
    ...
    if barrier_targets:
        ...
        bid = player.map.get_tile_entity_id(ore_pos)
        etype = player.map.get_tile_entity_type(ore_pos)
...
def run_extend_harvest_chain(player, ct: Controller, vc) -> None:
    ...
    if dest is not None and player.harvest_ore_pos is not None and ct.is_in_vision(player.harvest_ore_pos):
        new_pos = get_best_bridge_build_pos(
            ...
            player.map,
            vc,
            opposite_ore_mask=get_opposite_ore_mask(player.map, player.harvest_ore_type == ResourceType.RAW_AXIONITE),
        )
    ...
    dest_etype = player.map.get_tile_entity_type(dest) if dest is not None else None
    dest_team = player.map.get_tile_entity_team(dest) if dest is not None else None
    if dest is None:
    ...
    if (
        ...
        if (
            ...
            bid = player.map.get_tile_entity_id(dest)
        ...
        existing_bid = player.map.get_tile_entity_id(dest)
        existing_etype = player.map.get_tile_entity_type(dest)
        existing_team = player.map.get_tile_entity_team(dest)
    ...
    if not built_support_launcher:
        ...
        if player.harvest_ore_type == ResourceType.RAW_AXIONITE:
            ...
            if player.foundry_position_idxs is not None:
                for foundry_idx in player.foundry_position_idxs:
                    p = player.map.idx_to_pos(foundry_idx)
                    p_idx = foundry_idx
                    has_titanium = (
                        player.map.has_recent_conveyor_resource_idx(p_idx, ResourceType.TITANIUM)
                        or player.map.input_chain_reaches_resource_idx(p_idx, ResourceType.TITANIUM)
                    )
                    if not has_titanium and ct.is_in_vision(p):
                        p_bid = player.map.get_tile_entity_id(p)
                        p_etype = player.map.get_tile_entity_type(p)
                        if p_bid is not None and p_etype in CONVEYOR_TYPES:
                            has_titanium = ct.get_stored_resource(p_bid) == ResourceType.TITANIUM
                    if not has_titanium:
                        end_position_idxs.add(player.map.pos_to_idx(p))
        elif player.harvest_ore_type == ResourceType.TITANIUM and player.core_pos is not None:
            if player.foundry_pos is not None:
                if not player.map.is_single_input_foundry_idx(player.map.pos_to_idx(player.foundry_pos), player.my_team):
                    log(f"foundry at {player.foundry_pos} no longer needs titanium reroute -> redirecting to core")
                    player.foundry_pos = None
                else:
                    end_position_idxs = {player.map.pos_to_idx(player.foundry_pos)}
            else:
                ...
                if foundry_target is not None:
                    ...
                    for dx in range(-1, 2):
                        for dy in range(-1, 2):
                            end_position_idxs.add(player.map.pos_to_idx(Position(player.core_pos.x + dx, player.core_pos.y + dy)))
                    end_position_idxs.add(player.map.pos_to_idx(foundry_target))

        existing_bid = player.map.get_tile_entity_id(build_pos)
        existing_etype = player.map.get_tile_entity_type(build_pos)
        existing_team = player.map.get_tile_entity_team(build_pos)
        if existing_bid is not None and existing_etype in CONVEYOR_TYPES:
            next_pos = player.map.get_conveyor_output(build_pos)
            if next_pos is None:
                clear_state(player)
                return
            if existing_team == player.my_team:
                if is_core_tile(player.core_pos, next_pos):
                    ...
                    clear_state(player)
                    return
                next_etype = player.map.get_tile_entity_type(next_pos)
                next_team = player.map.get_tile_entity_team(next_pos)
                ...
                and (len(vc.enemy_units) == 0 or not player.map.feeds_ally_turret_idx(player.map.pos_to_idx(build_pos), player.my_team))
            ):
        ...
        if (
            ...
            conveyor_info, conveyor_is_fallback = player.map.get_best_conveyor_output_with_fallback_idx(
            ...
            if conveyor_info is None or conveyor_is_fallback:
                bridge_output_pos, bridge_is_fallback = player.map.get_best_bridge_output_with_fallback_idx(
                    build_pos, player.core_pos, ct, player.my_team, end_position_idxs=end_position_idxs, resource=chain_resource
                )
            ...
            if conveyor_info is not None:
                ...
                conv_target_idx = player.map.pos_to_idx(conv_target)
                feeds_foundry = (
                    (player.map.get_tile_entity_type(conv_target) == EntityType.FOUNDRY)
                ...
                if feeds_foundry:
                    ...
                    def _splitter_sides_clear(candidate_dir):
                        ...
                        for sd in CARDINAL_DIRECTIONS:
                            ...
                            if player.map.has_conflict_idx(chain_resource, player.map.pos_to_idx(side_pos), ct):
                                return False
                        return True
                    ...
                    for check_d in CARDINAL_DIRECTIONS:
                        ...
                        adj_etype = player.map.get_tile_entity_type(adj_pos)
                        adj_team = player.map.get_tile_entity_team(adj_pos)
                        if (
                            ...
                            feeder_output = player.map.get_conveyor_output(adj_pos)
...
def run_reroute_titanium(player, ct: Controller, vc) -> None:
    ...
    if ti_source is None:
        ti_source_idx = player.map.find_nearest_conveyor_with_resource_idx(
            player.map.pos_to_idx(my_pos),
            ResourceType.TITANIUM,
            my_team=player.my_team,
            target_foundry_idx=None if player.foundry_pos is None else player.map.pos_to_idx(player.foundry_pos),
        )
        ti_source = player.map.idx_to_pos(ti_source_idx) if ti_source_idx is not None else None
...
def run_intercept(player, ct: Controller, vc) -> None:
    ...
    bid = player.map.get_tile_entity_id(intercept_pos)
    if bid is None:
    ...
    bid_team = player.map.get_tile_entity_team(intercept_pos)
    bid_etype = player.map.get_tile_entity_type(intercept_pos)
    if (
    ...
    if player.map is not None and bid_team != player.my_team and player.map.feeds_ally_turret_idx(player.map.pos_to_idx(intercept_pos), player.my_team):
...
def run_defend(player, ct: Controller, vc) -> None:
    ...
    if ore_pos is None:
        clear_state(player)
        return
    if player.map.is_axionite_ore(ore_pos):
        clear_state(player)
        return
    if not ct.is_in_vision(ore_pos):
        return

    ore_bid = player.map.get_tile_entity_id(ore_pos)
    ore_etype = player.map.get_tile_entity_type(ore_pos)
