...
_HARVESTER_SABOTAGE_CACHE_KEY = None
_HARVESTER_SABOTAGE_GUNNER_MASKS = ()
...
def _harvester_sabotage_pressure(hpos: Position) -> bool:
    if map_info._bm_enemy_bots & map_info._bm_visible:
        return True
    ...
    # Compare squared distances without floats:
    # enemy_d2 < my_d2 * 1.5  <=>  enemy_d2 * 2 < my_d2 * 3.
    return _dist_sq(hpos, enemy_core) * 2 < _dist_sq(hpos, my_core) * 3


def _harvester_surrounded_by_enemy(hbit: int, enemy_team: int) -> bool:
    ring = (map_info.expand_chebyshev(hbit) & ~hbit) & map_info._board_mask
    return bool(ring) and (ring & ~enemy_team) == 0


def _harvester_sabotage_gunner_masks() -> tuple[int, ...]:
    """Per gunner direction, tiles where a gunner should replace sentinel.

    This only triggers for enemy harvesters whose whole surrounding ring is
    enemy-owned. Once we own one ring tile, the override naturally stops.
    """
    global _HARVESTER_SABOTAGE_CACHE_KEY, _HARVESTER_SABOTAGE_GUNNER_MASKS

    key = (
        rc.get_current_round(),
        map_info._struct_version,
        map_info._bm_enemy_bots & map_info._bm_visible,
    )
    if _HARVESTER_SABOTAGE_CACHE_KEY == key:
        return _HARVESTER_SABOTAGE_GUNNER_MASKS

    enemy_team = map_info._bm_team[1 - map_info._my_team_idx]
    harvesters = (
        map_info._bm_et[map_info._IDX_HARVESTER]
        & enemy_team
        & map_info._bm_seen_observed
    )
    masks = [0] * 8
    w = map_info._width

    while harvesters:
        hbit = harvesters & -harvesters
        harvesters ^= hbit
        if not _harvester_surrounded_by_enemy(hbit, enemy_team):
            continue

        hn = hbit.bit_length() - 1
        hpos = Position(hn % w, hn // w)
        if not _harvester_sabotage_pressure(hpos):
            continue
        ...
        for d in range(8):
            ...
            px = hpos.x - dx
            py = hpos.y - dy
            if 0 <= px < map_info._width and 0 <= py < map_info._height:
                masks[d] |= 1 << (px + py * w)

    _HARVESTER_SABOTAGE_CACHE_KEY = key
    _HARVESTER_SABOTAGE_GUNNER_MASKS = tuple(masks)
    return _HARVESTER_SABOTAGE_GUNNER_MASKS


def _forced_harvester_sabotage_gunner_dir(bit: int, gunner_masks: tuple[int, ...]) -> int:
    sabotage_masks = _harvester_sabotage_gunner_masks()
    for d in range(8):
        if bit & sabotage_masks[d] & gunner_masks[d]:
            return d
    return -1


def _forced_harvester_sabotage_gunners(gunner_masks: tuple[int, ...]) -> int:
    sabotage_masks = _harvester_sabotage_gunner_masks()
    result = 0
    for d in range(8):
        result |= sabotage_masks[d] & gunner_masks[d]
    return result
...
def get_best_direction(pos):
    ...
    forced_gunner_dir = _forced_harvester_sabotage_gunner_dir(bit, gunner_masks)
    if forced_gunner_dir >= 0:
        forced_score = _read_score(gun_planes_by_dir[forced_gunner_dir], n)
        forced_effective = forced_score + bias if forced_score > 0 else 1
        return directions[forced_gunner_dir], EntityType.GUNNER, forced_effective
...
def _get_attack_candidates():
    ...
    forced_gunners = (
        _forced_harvester_sabotage_gunners(gunner_masks)
        if can_afford_gun else 0
    )
    ...
    if max_score < MIN_ATTACK_SCORE+THREAT_PENALTY and not forced_gunners:
        return 0, 0
...
def attack_route_wait_gunner_anchor():
    """Best visible gunner placement worth waiting for, ignoring affordability."""
    enemy_idx = 1 - map_info._my_team_idx
    enemy_scorable = (
        map_info._bm_team[enemy_idx]
        & ~map_info._bm_et[map_info._IDX_ROAD]
    ...
    _sentinel_masks, gunner_masks = _placement_candidates()
    ...
    enemy_team_bm, threat = _round_cache_enemy_inputs()
    gun_planes_by_dir = _get_cached_gunner_per_dir(enemy_team_bm, threat, gunner_masks)
    forced_gunners = _forced_harvester_sabotage_gunners(gunner_masks) & visible

    gun_max = 0
    for d in range(8):
        mask = gunner_masks[d] & visible
        if mask:
            gun_max = max(gun_max, _max_score_in_mask(gun_planes_by_dir[d], mask))
    if gun_max < MIN_ATTACK_SCORE + THREAT_PENALTY and not forced_gunners:
        return None
    ...
    for d in range(8):
        ...
        if not mask:
            continue
        keep = mask & forced_gunners
        normal = mask & ~forced_gunners
        if normal:
            other = normal & ~good_conveyors
            good = normal & good_conveyors
            if other:
                keep |= _ge_threshold_mask(gun_planes_by_dir[d], threshold, other)
            if good:
                keep |= _ge_threshold_mask(gun_planes_by_dir[d], threshold + bias, good)
        while keep:
            lsb = keep & -keep
            keep ^= lsb
            forced_dir = _forced_harvester_sabotage_gunner_dir(lsb, gunner_masks)
            if forced_dir >= 0 and forced_dir != d:
                continue
            n = lsb.bit_length() - 1
            raw_score = _read_score(gun_planes_by_dir[d], n)
            score = raw_score + _selection_bias_for_bit(lsb) if raw_score > 0 else raw_score
            if forced_dir >= 0 and score <= 0:
                score = 1
            pos = Position(n % map_info._width, n // map_info._width)
            dist2 = my_pos.distance_squared(pos)
    ...
    if best_pos is None:
        return None
    return best_pos, best_dir, best_score
