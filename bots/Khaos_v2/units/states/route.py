...
def _attack_route_wait_anchor():
    attack = sys.modules.get("units.states.attack")
    if attack is None:
        import units.states.attack as attack
    return attack.attack_route_wait_gunner_anchor()

def _try_wait_for_attack_turret() -> bool:
    anchor = _attack_route_wait_anchor()
    if anchor is None:
        return False

    target, direction, _score = anchor
    reserve = map_info.builder_ti_reserve()

    dist2 = map_info._my_pos.distance_squared(target)
    if dist2 == 0 or dist2 > 2:
    ...
    gunner_ready = ti_have >= rc.get_gunner_cost()[0] + reserve
    target_type = map_info.type_at(target.x, target.y)
    target_team = map_info.team_at(target.x, target.y)

    if gunner_ready:
        if target_type is not None and target_team == map_info._my_team:
            if not map_info.has_builder_bot(target) and rc.can_destroy(target):
                rc.destroy(target)
                map_info.update_at(target)
        if rc.can_build_gunner(target, direction):
            rc.build_gunner(target, direction)
            map_info.update_at(target)
        return True

    if (
        target_type is None
        and rc.can_build_road(target)
        and ti_have >= rc.get_road_cost()[0] + reserve
    ):
        rc.build_road(target)
        map_info.update_at(target)
    return True
...
def run():
    ...
    while candidates:
        ...
        is_attack_route = cand_attack_route
        break
    ...
    if is_raw_ax and (foundry_sites & tc0_bit) and target_conveyor[0] == target_conveyor[1]:
        ...
        if rc.get_action_cooldown() == 0 and rc.get_global_resources()[0] >= rc.get_foundry_cost()[0] + map_info.builder_ti_reserve():
            ...
            if rc.can_build_foundry(target_conveyor[0]) and rc.get_global_resources()[0] >= rc.get_foundry_cost()[0] + map_info.builder_ti_reserve():
                ...
                map_info.update_at(target_conveyor[0])
        return
    if is_attack_route and _try_wait_for_attack_turret():
        return
    near_enemy = False
