...
def run():
    ...
    # Scale max builders with time and resources
    # Early game: conservative (3-4 builders)
    # Mid game with resources: ramp up to expand economy
    max_spawn = 4
    if round_num < 100:
        max_spawn = 3
    elif titanium > 600 and round_num > 200:
        max_spawn = 6
    elif titanium > 800 and round_num > 400:
        max_spawn = 8
    ...
    # Normal spawning logic — also spawn when we have excess titanium to expand economy
    should_spawn = (
        num_spawned < max_spawn or
        rc.get_hp() < 500 or
        rc.get_global_resources()[0] > 900 + rc.get_scale_percent() or
        (titanium > 500 and round_num > 150 and num_spawned < max_spawn + 2)
    )
    if should_spawn:
