"""Test side asymmetry: track Hades wins separately when playing as bot_a vs bot_b."""
...
def main():
    if len(sys.argv) < 2:
        print("usage: python _test_side_asymmetry.py <variant_path>...")
        sys.exit(1)
    variants = sys.argv[1:]
    ...
    for variant in variants:
        for opp in OPPONENTS:
            opp_path = f"bots/{opp}"
            for m in maps:
                jobs.append((variant, opp_path, m, SEED, variant, opp, "A"))
                jobs.append((opp_path, variant, m, SEED, variant, opp, "B"))

    print(f"Total matches: {len(jobs)}")
    # results[(variant, side)] = [w, l, d, e]
    results: dict[tuple[str, str], list[int]] = defaultdict(lambda: [0, 0, 0, 0])
    completed = 0
    t0 = time.perf_counter()

    def submit(bot_a, bot_b, m, seed, variant, opp, side):
        return run_match(bot_a, bot_b, m, seed), variant, opp, bot_a, bot_b, side

    with ThreadPoolExecutor(max_workers=THREADS) as ex:
        futs = [ex.submit(submit, *j) for j in jobs]
        for f in as_completed(futs):
            mr, variant, opp, bot_a, bot_b, side = f.result()
            completed += 1
            entry = results[(variant, side)]
            ...
            print(f"  [{completed}/{len(jobs)}] [{side}] {variant.split('/')[-1]} vs {opp}: t={mr.turn} {mr.elapsed_s:.0f}s | ETA {eta:.0f}s", flush=True)

    print()
    print("=" * 80)
    print(f"{'Variant':<35} | {'Side A':<14}  {'Side B':<14}  TOTAL")
    print("=" * 80)
    for variant in variants:
        row = []
        tot_w = tot_l = tot_d = tot_e = 0
        for side in ["A", "B"]:
            w, l, d, e = results[(variant, side)]
